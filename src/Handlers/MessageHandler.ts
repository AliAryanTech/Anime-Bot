...
export default class MessageHandler {
	...
	handleMessage = async (M: ISimplifiedMessage): Promise<void> => {
		...
		if (!command)
			return void M.reply(
				`𝗪𝗿𝗼𝗻𝗴  𝗖𝗼𝗺𝗺𝗮𝗻𝗱, 𝗖𝗵𝗼𝗺𝗶𝗲! 𝗨𝘀𝗲 𝘁𝗵𝗲 𝗳𝗼𝗹𝗹𝗼𝘄𝗶𝗻𝗴 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 𝘁𝗼 𝗮𝗰𝗰𝗲𝘀𝘀 𝗭𝗶𝗺 𝗕𝗼𝘁 *${this.client.config.prefix}help*.`
			);
		...
		if (command.config?.adminOnly && !M.sender.isAdmin)
			return void M.reply(
				`This command is only meant for the group admins, 𝗖𝗵𝗼𝗺𝗶𝗲!`
			);
		try {
